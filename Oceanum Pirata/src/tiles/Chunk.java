...
public class Chunk {
	...
	private int Xid, Yid;
	private Tile[][] tiles;
	private Rectangle body = new Rectangle(), north = new Rectangle(), south = new Rectangle(), west = new Rectangle(),
			east = new Rectangle();
	...
	public void Update() {
		...
		body.setBounds((Xid * 3200) - (Display.getWidth() / 2), (Yid * 3200) - (Display.getHeight() / 2),
				3200 + (Display.getWidth()), 3200 + (Display.getHeight()));
		// Screen.DrawQuadGameTex(Textures.CHEST, body.getX(), body.getY(),
		...
		if (this.north.intersects(Main.GAME.p.getCollider())) {
			if (Chunk.getChunkAt(this.getXid() * 3200, this.getYid() * 3200 - 10) == null) {
				float[][] noise = NoiseGenerator.generateOctavedSimplexNoise(this.Xid * 100, (this.Yid * 100) - 100,
						100, 100, 8, 0.3f, 0.005f);
				Main.GAME.ActiveChunksToAdd.add(new Chunk(this.getXid(), this.getYid() - 1,
						ChunkGenerator.FromSimplexNoise(noise, this.getXid(), this.getYid() - 1)));
			}
		}
		if (this.west.intersects(Main.GAME.p.getCollider())) {
			if (Chunk.getChunkAt(this.getXid() * 3200 - 10, this.getYid() * 3200) == null) {
				float[][] noise = NoiseGenerator.generateOctavedSimplexNoise((this.getXid() * 100 - 100),
						this.getYid() * 100, 100, 100, 8, 0.3f, 0.005f);
				Main.GAME.ActiveChunksToAdd.add(new Chunk(this.getXid() - 1, this.getYid(),
						ChunkGenerator.FromSimplexNoise(noise, this.getXid() - 1, this.getYid())));
			}
		}
		if (this.south.intersects(Main.GAME.p.getCollider())) {
			if (Chunk.getChunkAt(this.getXid() * 3200, this.getYid() * 3200 + 3210) == null) {
				float[][] noise = NoiseGenerator.generateOctavedSimplexNoise(this.getXid() * 100,
						(this.getYid() * 100) + 100, 100, 100, 8, 0.3f, 0.005f);
				Main.GAME.ActiveChunksToAdd.add(new Chunk(this.getXid(), this.getYid() + 1,
						ChunkGenerator.FromSimplexNoise(noise, this.getXid(), this.getYid() + 1)));
			}
		}
	...
	public static void setTileAt(Tile t, float x, float y) {
		Tile tochange = Chunk.getTileAt(x, y);
